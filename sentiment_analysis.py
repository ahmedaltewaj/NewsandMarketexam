...
def main():
    ...
    ##lexicon analysis
    print("Running Lexicon Analysis...")
    start_time = time.time()
    
    def get_lexicon_sentiment(title):
        ...
        while i < n:
            ...
            for length in range(max_phrase_len, 0, -1):
                ...
                if phrase in lexicon_dict:
                    score += lexicon_dict[phrase]; count += 1; i += length; match_found = True; break
            if not match_found: i += 1
        return score / count if count > 0 else 0.0

    sample_data['lexicon_sentiment'] = sample_data['Title'].apply(get_lexicon_sentiment)
    ...
    sample_data['transformer_sentiment'] = [r['score'] if r['label'] == 'positive' else -r['score'] if r['label'] == 'negative' else 0 for r in results]
    ...
    def get_metrics(df, s_col, t_col):
        ...
        try: corr = np.nan_to_num(np.corrcoef(df[s_col], df[t_col])[0, 1])
        except: corr = 0.0
        return acc, corr
    ...
    ##Comparison Logic
    speed_multiple = (advanced_avg_time / lexicon_avg_time) if lexicon_avg_time > 0 and advanced_avg_time > lexicon_avg_time else 1.0
    faster_method = "Lexicon" if speed_multiple > 1.0 else "Same speed"
    
    if adv_acc_d > lex_acc_d: winner, explanation = "TRANSFORMER WINS!", "understanding context is better than not"
    elif lex_acc_d > adv_acc_d: winner, explanation = "LEXICON WINS!", "having domain knowledge is better than not"

    adv_vs_random = adv_acc_d - 0.5
    ##to divide the significance, to see if there is 1) statistical significance, 2) practical signifance or both
    market_insight = "Sentiment has predictive power!" if adv_vs_random > 0 else "Market is mostly efficient" if adv_vs_random > -0.05 else "Market beats sentiment analysis"
    ...
    print(f"\nComparison:\n{faster_method} is {speed_multiple:.1f}x faster\nTransformer is {((adv_acc_d - lex_acc_d) * 100):+.1f}% more accurate (Daily)")
    ...
    if not disagreements.empty:
        ...
        if not interesting.empty:
            ...
            print("saved to 'model_disagreements.csv'")
        else: print("No headline disagreement or error")
    else: print("No headline disagreement or error")

    ##save full results in same path
    analysis_df.to_csv(os.path.join(base_dir, 'sentiment_analysis_results.csv'), index=False)
    return {'lexicon_speed_ms': lexicon_avg_time, 'lexicon_accuracy': lex_acc_d, 'advanced_speed_ms': advanced_avg_time, 'advanced_accuracy': adv_acc_d, 'winner': winner, 'market_efficiency': market_insight}
